...
*/
// 所有通过stub代理对象调用的rpc方法，都走到这里了，统一做rp方法调用的数据序列化和网络发送
                        ...
                        google::protobuf::Closure* done)
{
    ...
    send_rpc_str.insert(0, (char*)&header_size, 4); // header_size
    ...
    {
        char errmsg[512] = {0};
        sprintf(errmsg, "connect socket error! errno: %d", errno);
        controller->SetFailed(errmsg);
        return;
    }
    ...
    {
        char errmsg[512] = {0};
        sprintf(errmsg, "recive error! errno: %d", errno);
        controller->SetFailed(errmsg);
        return;
    }
